...
class make_test:
    ...
    def assert_dt_abs(self, date_dt, year=None, month=None, day=None, hour=None,
                      minute=None, second=None, tz_text=None):
        if year is not None:
            assert year == date_dt.year
        if month is not None:
            assert month == date_dt.month
        if day is not None:
            assert day == date_dt.day
        if hour is not None:
            assert hour == date_dt.hour
        if minute is not None:
            assert minute == date_dt.minute
        if second is not None:
            assert second == date_dt.second
        if tz_text is not None:
            assert tz_text == str(date_dt.tzinfo)
