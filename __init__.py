...
class fw:
    ...
    def get_keyword_names(self):
        names = keyword.KeywordInfo().get_qualified_keywords()
        return [conventions.KeywordNameConventions().convert_name(n, 'method') for n in names]

    def get_keyword_arguments(self, name: str):
        name = conventions.KeywordNameConventions().convert_name(name, 'keyword')
    ...
    def run_keyword(self, name: str, args: list, kwargs: dict):
        name = conventions.KeywordNameConventions().convert_name(name, 'keyword')
        return execution.Runner(self).run_kw(name, args, kwargs)
