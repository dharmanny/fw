...
class TestMakeDate:
    ...
    def assert_dt_abs(self, date_dt, year=None, month=None, day=None, hour=None,
        ...
        if year is not None:
            assert year == date_dt.year, 'Year should have been {year}'.format(year=year)
        if month is not None:
            assert month == date_dt.month, 'Month should have been {month}'.format(month=month)
        if day is not None:
            assert day == date_dt.day, 'Day should have been {day}'.format(day=day)
        if hour is not None:
            assert hour == date_dt.hour, 'Hour should have been {hour}'.format(hour=hour)
        if minute is not None:
            assert minute == date_dt.minute, 'Minute should have been {min}'.format(min=minute)
        if second is not None:
            assert second == date_dt.second, 'Second should have been {sec}'.format(sec=second)
        if tz_text is not None:
            assert tz_text == str(date_dt.tzinfo), 'Timezone should be {tz}'.format(tz=tz_text)
    ...
    def assert_dt_rel(self, date_dt, start, stop, year=False, month=False, day=False, hour=False,
        ...
        if year:
            assert start.year <= date_dt.year <= stop.year, 'Year should have been between {t1} and ' \
                                                            '{t2}'.format(t1=start.year, t2=stop.year)
        if month:
            assert start.month <= date_dt.month <= stop.month,'Month should have been between {t1} and ' \
                                                              '{t2}'.format(t1=start.month, t2=stop.month)
        if day:
            assert start.day <= date_dt.day <= stop.day, 'Day should have been between {t1} and ' \
                                                         '{t2}'.format(t1=start.day, t2=stop.day)
        if hour:
            assert start.hour <= date_dt.hour <= stop.hour, 'Hour should have been between {t1} and ' \
                                                            '{t2}'.format(t1=start.hour, t2=stop.hour)
        if minute:
            assert start.minute <= date_dt.minute <= stop.minute, 'Minute should have been between {t1} and ' \
                                                                  '{t2}'.format(t1=start.minute, t2=stop.minute)
        if second:
            assert start.second <= date_dt.second <= stop.second, 'Second should have been between {t1} and ' \
                                                                  '{t2}'.format(t1=start.second, t2=stop.second)
        if tz_text is not None:
            assert str(date_dt.tzinfo) == tz_text, 'Timezone should be {tz}'.format(tz=tz_text)
...
class TestMakeDateOrReturn:
    def setup_method(self):
        self.dp = fw_dt.DateParser()

    assert_dt_abs = TestMakeDate.assert_dt_abs
    assert_dt_rel = TestMakeDate.assert_dt_rel
    ...
    def test_negative(self):
        ...
        assert 'invalid_date' == date_dt, 'The input should have been returned'
...
class TestMakeDateRange:
    ...
    def test_positive_till(self):
        rng = list(self.dp.make_date_range('UTC 2019/01/01', till_date='UTC 2029/01/01', size=1, unit='year'))
        ...
        assert 10 == len(rng), 'The range should consist of 10 items.'

    def test_positive_through(self):
        rng = list(self.dp.make_date_range('UTC 2019/01/01', through_date='UTC 2029/01/01', size=1, unit='year'))
        self.assert_dt_abs(rng[0], year=2019, month=1, day=1, hour=0, minute=0, second=0)
        self.assert_dt_abs(rng[-1], year=2029, month=1, day=1, hour=0, minute=0, second=0)
        assert 11 == len(rng), 'The range should consist of 11 items.'

    def test_month(self):
        rng = list(self.dp.make_date_range('UTC 2019/01/01', through_date='UTC 2019/05/01', size=1, unit='month'))
        self.assert_dt_abs(rng[0], year=2019, month=1, day=1, hour=0, minute=0, second=0)
        self.assert_dt_abs(rng[-1], year=2019, month=5, day=1, hour=0, minute=0, second=0)
        assert 5 == len(rng), 'The range should consist of 5 items.'

    def test_day(self):
        rng = list(self.dp.make_date_range('UTC 2019/01/01', through_date='UTC 2019/01/05', size=1, unit='day'))
        self.assert_dt_abs(rng[0], year=2019, month=1, day=1, hour=0, minute=0, second=0)
        self.assert_dt_abs(rng[-1], year=2019, month=1, day=5, hour=0, minute=0, second=0)
        assert 5 == len(rng), 'The range should consist of 5 items.'

    def test_hour(self):
        rng = list(self.dp.make_date_range('UTC 2019/01/01 1', through_date='UTC 2019/01/01 5', size=1, unit='hour'))
    ...
    def test_minute(self):
        rng = list(self.dp.make_date_range('UTC 2019/01/01 1:1', through_date='UTC 2019/01/01 1:5', size=1,
                                      unit='minute'))
    ...
    def test_second(self):
        rng = list(self.dp.make_date_range('UTC 2019/01/01 1:1:1', through_date='UTC 2019/01/01 1:1:5', size=1,
                                      unit='second'))
